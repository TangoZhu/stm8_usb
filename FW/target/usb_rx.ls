   ...
   4                     
   5  0000               L_Bit0_Begin:
   6  0000 906314        	cpl		($14,y)					; сигнал синхронизации
   7  0003 0401          	srl		($01, SP)
   8  0005 90f6          	ld		a,(y)
   9  0007 2604ac8c008c  	jreq	L_End_Rx				; выход, если input = 0 (конец пакета)
 ...
 102                     
 103  0094 725f500f      	clr		20495
 104                     
 105  0098 35fb5255      	mov		21077,#251 				; сброс флага прерывания таймера
 106                     
 107  009c 72115250      	bres	21072,#0				; стоп таймер
 108  00a0 725f525e      	clr		21086					; сброс счетного регистра
 ...
 117  00a9               _usb_rx:
 118                     
 119  00a9 ae00b7        	ldw		x, #L_Delay_Begin	; адрес начала таблицы задержек
 120  00ac 72bb525e      	addw	x, 21086			; + смещение (значение таймера)
 121  00b0 a300e1        	cpw		x, #L_Delay_End		; проверка выхода за границу таблицы (защита!)
 122  00b3 2401          	jruge	L_Error
 123  00b5 fc            	jp		(x)					; переход по таблице задержек
 124                     
 125  00b6               L_Error:
 126                     
 127  00b6 80            	iret ; аврийный выход
 128                     
 129  00b7               L_Delay_Begin:
 130  00b7 9d            	nop
 131  00b8 9d            	nop
 132  00b9 9d            	nop
 133  00ba 9d            	nop
 ...
 171  00e0 9d            	nop
 172                     
 173  00e1               L_Delay_End:
 174                     
 175  00e1 ae0000        	ldw		x, #_usb_rx_buffer		; адрес начала буфера
 176  00e4 90ae500b      	ldw		y, #20491 				; адрес входного сигнала
 177  00e8 a60e          	ld		a, #$0E					; максимальное кол-во принимаемых байт (защита!)
 178  00ea 88            	push	a
 179  00eb a680          	ld		a, #$80
 180  00ed 88            	push	a
 181                     
 182  00ee ac000000      	jra		L_Bit0_Begin
